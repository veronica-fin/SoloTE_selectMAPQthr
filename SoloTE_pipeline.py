...
def processPerChrom(chromosome,inputfile,outdir,outbasename):
    ...
    if os.path.exists(countsname) == True:
        if  os.path.getsize(countsname)>0:
            ...
            print(f'Counts for chromosome {chromosome} are being regenerated in process: {os.getpid()}')
    else:
    ...
    # change awk command to be more flexible to tag order
    awk_script = r"""
    BEGIN { FS=OFS="\t" }
    {
        gn="-"; cb="-"; ub="-";
        ...
        # We keep "AmbiguousTE" and "Gene;SoloTE" by printing 'gn' exactly as is
        if (gn != "-" && cb != "-" && ub != "-") {
            print gn, cb, ub
        }
    }
    ...
    cmd = f"samtools view {inputfile} {chromosome} | awk '{awk_script}' > {temp_countsfile}"
    os.system(cmd)
...
#    cmd="samtools view --threads "+cpus+" -O BAM -o "+te_bam+" -L "+TE_bed+" "+bam_without_genes
    # index input bam
    cmd="samtools index "+inputbam
    print(cmd)
    os.system(cmd)
    # We add the -M flag here to ensure mates of TE-overlapping reads are included
    if mode == "Dual":
       cmd="samtools view -@ "+cpus+" -M -O BAM -o "+te_bam+" -L "+TE_bed+" -e '(exists([CB]) && exists([UB]) && [CB]!=\"-\" && [UB]!=\"-\")' "+inputbam
    else:
       cmd="samtools view -@ "+cpus+" -M -O BAM -o "+te_bam+" -L "+TE_bed+" -e '(exists([CB]) && exists([UB]) && [CB]!=\"-\" && [UB]!=\"-\") && (!exists([GN]) || [GN]==\"-\")' "+inputbam
    print(cmd)
    os.system(cmd)
...
if os.path.exists(annotated_te_bam):
    ...
    annotateBAMpath=SoloTE_Home+"/annotateBAM_strandSpecific_paired2.py"
...
# changed check from full_bam to final_bam
...
# Load all merged counts from your per-chromosome files
allcounts = pd.read_table(allcountsfile, header=None, names=['feature', 'barcode', 'umi'])

# 1. Identify Chimeric (contains both a Gene and SoloTE)
is_chimeric = allcounts['feature'].str.contains(";") & allcounts['feature'].str.contains("SoloTE")

# 2. Identify Pure TE (Starts with SoloTE, no semicolon)
is_pure_te = allcounts['feature'].str.startswith("SoloTE") & ~allcounts['feature'].str.contains(";")

# 3. Identify Pure Gene (No SoloTE at all)
is_pure_gene = ~allcounts['feature'].str.contains("SoloTE")

# --- SEPARATE THE DATA ---
chimericcounts = allcounts[is_chimeric].copy()
tecounts  = allcounts[is_pure_te].copy()
genecounts = allcounts[is_pure_gene].copy()

# Save the Chimeric reads to a dedicated file for analysis
chimericcounts.to_csv(outdir + "/" + outbase + ".chimeric_counts.tsv", sep="\t", index=False)

tecounts2 = tecounts['feature'].str.split("|", expand=True)
if tecounts2.shape[1]==2:
    ...
    tecounts2[5]=None
    tecounts2[6]=None
# Fill missing name column (column 4) with subfamily column (column 1)
tecounts2[4] = tecounts2[4].fillna(tecounts2[1])
# tecounts2.loc[tecounts2[4].isnull(),4] = tecounts2.loc[tecounts2[4].isnull(),1]

te_annotation = tecounts2[4].str.split(":", expand=True)


te_counts_reset = tecounts.reset_index(drop=True)
te_anno_reset = te_annotation.reset_index(drop=True)
# Combine original TE data with the Subfamily/Family/Class split
te_final_df = pd.concat([te_counts_reset, te_anno_reset], axis=1)
te_final_df.columns = ['TE_original_id', 'barcode', 'umi', 'Subfamily', 'Family', 'Class']
#te_final_df.replace("\?", "", inplace=True, regex=True) # don't remove question marks

locus_tes = te_final_df[te_final_df['TE_original_id'].str.contains(r"SoloTE\|chr")]

legacy_df = te_final_df.groupby(['TE_original_id','barcode'],as_index=False)['umi'].sum()
locus_df = locus_tes.groupby(['TE_original_id','barcode'],as_index=False)['umi'].sum()
class_df = te_final_df.groupby(['Class','barcode'],as_index=False)['umi'].sum()
subfamily_df = te_final_df.groupby(['Subfamily','barcode'],as_index=False)['umi'].sum()
family_df = te_final_df.groupby(['Family','barcode'],as_index=False)['umi'].sum()
family_df.replace('',numpy.nan,inplace=True)
family_df.dropna(subset=['Family'], inplace=True)

gene_df = genecounts.groupby(['feature','barcode'],as_index=False)['umi'].sum()
gene_df.columns = ['Gene_id','barcode','umi']
# Chimeric reads (The "Double Names")
chimeric_df = chimericcounts.groupby(['feature','barcode'], as_index=False)['umi'].sum()
chimeric_df.columns = ['Gene_id', 'barcode', 'umi']
# COMBINE Genes + Chimerics: This ensures "Gene;SoloTE" is in the matrix
gene_final_base = pd.concat([gene_df, chimeric_df])

for df in [legacy_df, locus_df, class_df, family_df, subfamily_df]:
    df.columns = ['Gene_id', 'barcode', 'umi']
...
gene_legacytes = pd.concat([gene_final_base, legacy_df])
gene_locustes = pd.concat([gene_final_base, locus_df])
gene_classtes = pd.concat([gene_final_base, class_df])
gene_familytes = pd.concat([gene_final_base, family_df])
gene_subfamilytes = pd.concat([gene_final_base, subfamily_df])
...
filelist = [
    outbase + "_legacytes.txt",
    outbase + "_locustes.txt",
    outbase + "_classtes.txt",
    outbase + "_familytes.txt",
    outbase + "_subfamilytes.txt"
]
...
for countsfile in filelist:
    ...
    ret = os.system(cmd)
    if ret != 0:
        print(f"[ERROR] Rscript failed for {countsfile}, skipping matrix move")
        continue
    if os.path.exists(mtx_outname):
        shutil.move(mtx_outname, finaldir + "/" + mtx_outname)
    else:
        print(f"[WARNING] Expected output {mtx_outname} not found after Rscript")



total_gene_counts = gene_df['umi'].sum()
total_te_counts = legacy_df['umi'].sum()
total_locuste_counts = locus_tes['umi'].sum()
